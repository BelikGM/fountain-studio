 ...
 * и треугольные островки, и звезда к празднику. Точки раскладываются РАВНОМЕРНО
 * ПО ПЕРИМЕТРУ фигуры, а не по вершинам: тогда одно и то же число форсунок
 * годится любой фигуре, а на вершины они попадают сами собой, когда количество
 * кратно числу сторон.
 */
...
/** Вершины контура фигуры (замкнутого). */
function shapeOutline(shape: LayoutShape, radius: number, aspect: number): { x: number; y: number }[] {
  const pt = (a: number, r: number): { x: number; y: number } => ({
    x: r * Math.cos(a),
    y: r * Math.sin(a),
  });
  switch (shape) {
    case 'square': {
      const s = radius; // «радиус» квадрата — половина стороны
      return [
        { x: -s, y: -s },
        { x: s, y: -s },
        { x: s, y: s },
    ...
    case 'rect': {
      const a = radius;
      const b = radius * Math.max(0.1, aspect);
      return [
        { x: -a, y: -b },
        { x: a, y: -b },
        { x: a, y: b },
    ...
    case 'triangle':
      // Вершиной вверх: так треугольник читается сразу.
      return [0, 1, 2].map((i) => pt(Math.PI / 2 + (i * 2 * Math.PI) / 3, radius));
    case 'star': {
...
export function shapePositions(
  ...
  centerY = 0,
  startDeg = 0,
  /** Для прямоугольника: отношение половины высоты к половине ширины. */
  aspect = 0.6,
): { x: number; y: number }[] {
