...
export interface PlaybackState {
  ...
  pausedAll: boolean;
  /**
   * Всё погашено поверх любых слоёв: 'off' — «Выключить» по расписанию, до
   * следующего включения; 'transition' — гашение перед записью расписания.
   */
  dark?: 'off' | 'transition' | null;
  /**
   * На каком пункте каждый плейлист остановился в прошлый раз — для запуска
   * «с места остановки». Движок хранит это в папке объекта, чтобы место не
   * терялось при перезапуске.
   */
...
export interface TelegramStatus {
  ...
  /**
   * В какие дни суточный отчёт ДОШЁЛ (даты «ГГГГ-ММ-ДД», последние 30).
   *
   * Молчание бота само по себе ничего не значит: может, на объекте всё
   * спокойно, а может, бот умер ещё неделю назад. По этому списку видно, что
   * связь была каждый день.
   */
  reportDays: string[];
}
...
export type ClientMessage =
   ...
   * каждые десять секунд роняет насосы и свет в 0 — проверить нечего. Раньше
   * ради этого выключали `failsafe.enabled` в самом объекте, но объект едет на
   * фонтан вместе с этой галочкой: там гашение — защита, и выключенным оно
   * приезжать не должно. Поэтому переключатель живёт в настройках программы:
   * переживает перезагрузку страницы и перезапуск, но остаётся на столе.
   */
...
export type ServerMessage =
  ...
  /**
   * Итог применения вселенных и такта — ТОМУ редактору, который применял.
   *
   * Раньше редактор писал «✔ применено и сохранено» сам, не дожидаясь
   * движка. Если связи в этот момент не было (движок перезапускался) или
   * движок правку отклонил, человек видел галочку, а вселенной не было нигде.
   * Теперь галочка — только по этому ответу.
   */
  | { type: 'configResult'; ok: boolean; message: string; changes: string[] }
