 ...
 *
 * Голое «Вселенная» (или «Линия», «Universe») без номера — тоже не имя: на
 * кнопках «Отладки» выходило «Вселенная 1 · Вселенная», и на ноутбуке ряд
 * кнопок занимал пол-экрана (заказчик 24.09.2026).
 */
function isAutoLabel(id: number, label: string): boolean {
  const m = /^\s*(?:линия|вселенная|universe)\s*(\d+)?\s*$/i.exec(label);
  return m !== null && (m[1] === undefined || Number(m[1]) === id);
}
