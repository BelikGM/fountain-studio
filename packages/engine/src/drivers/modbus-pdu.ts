...
/**
 * Чтение input-регистров. Нужно датчику ветра: многие анемометры с RS-485
 * отдают скорость именно так (функция 04), а не holding-регистром, как ПЧ.
 */
...
export function checkException(pdu: Buffer, fc: number): Buffer {
  if (pdu.length > 0 && pdu.readUInt8(0) === (fc | 0x80)) {
    ...
    throw new Error(`Modbus-исключение от прибора, код ${code}`);
  }
  return pdu;
}

export function pduReadInputRegisters(address: number, count = 1): Buffer {
  const pdu = Buffer.alloc(5);
...
export function parseInputRegisters(pdu: Buffer): number[] {
  checkException(pdu, FC_READ_INPUT_REGISTERS);
