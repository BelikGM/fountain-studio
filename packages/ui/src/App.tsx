...
const TABS: { id: Tab; label: string; full: string }[] = [
  ...
  { id: 'sequences', label: 'Секвенсоры', full: 'Секвенсоры — сцены друг за другом по кругу или один раз; в FontanPlay это называлось макросами' },
  { id: 'show', label: 'Шоу', full: 'Шоу — один номер под одну песню: сцены и кривые на шкале времени' },
  { id: 'playlists', label: 'Плейлисты', full: 'Плейлисты — несколько шоу подряд: программа на день или на вечер' },
  ...
  { id: 'network', label: 'Диагностика', full: 'Диагностика — исправность оборудования: отвечают ли Art-Net ноды и приборы' },
      ...
      */}
      ...
      <footer className="statusbar">
        ...
        */}
        {noProject ? (
          ...
          <>
            <span data-hint={`Такт отправки: движок шлёт приборам новый кадр каждые ${stats.intervalMs} мс — ${Math.round(1000 / Math.max(1, stats.intervalMs))} раз в секунду`}>
              такт {stats.intervalMs} мс
            </span>
            <span
              data-hint={
      ...
      const manualText = (): string => {
        ...
        return `шоу не играет · вручную: ${where}, ${manual.what} — ${when}`;
              ...
              const dark =
                playback.dark === 'off'
                  ? 'стоп по расписанию — всё погашено до следующего запуска'
