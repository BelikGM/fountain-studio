...
export function ManualBlocked({ engine }: { engine: EngineConnection }) {
  ...
  const benchOn: Strip['action'] = {
    label: 'Режим наладки',
    hint: 'На ЭТОМ компьютере аварийное гашение перестанет срабатывать, и приборами можно управлять руками без интерфейса DMX. Настройка программы: в объект не попадёт, на фонтане гашение останется включённым. Переживает перезагрузку страницы и перезапуск программы.',
  ...
  const strips: (Strip | null)[] = [
    ...
    engine.connected && failsafe?.active
      ? {
          danger: true,
          text: `Работает аварийное гашение (${failsafe.reason || 'причина не указана'}). Насосы, клапаны и свет уходят в 0 каждый такт — поэтому ползунок и «падает». На столе включите режим наладки: гашение перестанет срабатывать здесь, в объекте останется включённым.`,
    ...
    project && !project.failsafe.enabled
      ? {
          danger: false,
          text: 'В объекте выключено аварийное гашение. Эта настройка уедет на фонтан вместе с проектом — там она нужна включённой.',
          action: {
            label: 'Включить в объекте',
            hint: 'Вернуть аварийное гашение в настройках объекта',
