 ...
 *  · «Вход» — чужой Art-Net, пойманный на линии (внешний пульт, вторая
...
const STAGES: { id: Stage; label: string; hint: string }[] = [
  { id: 'out', label: 'Выход', hint: 'Что насчитал движок: воспроизведение и ручные фейдеры, потом калибровка, ветер и служебный свет' },
  ...
  { id: 'in', label: 'Вход', hint: 'Чужой Art-Net, пойманный на линии: внешний пульт, вторая программа, дублирующий источник' },
];
...
const ROLES: { id: RoleFilter; label: string; color: string; hint: string }[] = [
  ...
  { id: 'pump', label: 'Насосы', color: '#4fa3ff', hint: 'Адреса приборов с видом «насос» в профиле — высота струи' },
  { id: 'valve', label: 'Клапаны', color: '#ff6b6b', hint: 'Адреса приборов с видом «клапан» — вода есть / воды нет' },
  { id: 'lamp', label: 'Свет', color: '#ffd166', hint: 'Адреса приборов с видом «свет» — прожекторы, цвет и яркость' },
  {
    id: 'other',
    label: 'Прочее',
    color: '#8f9aa6',
    hint: 'Приборы, которым в профиле выбран вид «прочее»: не насос, не клапан и не свет — дым, вентилятор, реле, пламя. Свободные адреса сюда НЕ попадают, они только под «Все»',
  },
];
  ...
  return (
    <main className="view">
      <section className="panel">
        <h2>Поток DMX</h2>
        <p className="dim">
          Что уходит в линию и что приходит обратно. Три участка одного пути:{' '}
          <b>Выход</b> — что насчитал движок; <b>В кабель</b> — то же самое после переадресации, буквально
          байты в проводе; <b>Вход</b> — чужой Art-Net, пойманный на линии (внешний пульт, вторая
          программа).
        </p>
        <p className="dim">
  ...
  return (
    <section className="panel">
      ...
      {network.rdmDevices.length === 0 ? (
        <p className="dim">Приборов RDM на линии не найдено.</p>
      ) : (
        <table className="table">
          <thead>
            <tr>
          ...
          <tbody>
            {network.rdmDevices.map((d) => (
              <tr key={d.uid} className={d.lost ? 'row-lost' : undefined}>
                ...
                <td className={d.lost ? 'error-text' : 'dim'}>{d.lost ? 'пропал с линии' : 'отвечает'}</td>
              </tr>
            ))}
          </tbody>
        </table>
