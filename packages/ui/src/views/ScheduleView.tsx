...
const ACTION_LABEL: Record<ScheduleAction['type'], string> = {
  ...
  playlist: 'Плейлист',
  scene: 'Сцена',
  pause: 'Пауза — замереть',
  stopAll: 'Стоп — в покой',
  off: 'Выключить — всё в 0',
};

const ACTION_HINT =
  'Макрос, группа, шоу, плейлист, сцена — запустить; то, что играло, остановится.\n' +
  '«Пауза» — картина замирает как есть.\n' +
  '«Стоп» — программы останавливаются, фонтан в покое: горят «Сцена, когда ничего не играет» и служебный свет, если заданы в «Настройках».\n' +
  '«Выключить» — гаснет всё, включая их, до следующего включения — записью или руками.';
...
function ScheduleEditor({
  ...
  const setAction = (e: ScheduleEntry, type: ScheduleAction['type']): void => {
    if (type === 'pause' || type === 'stopAll' || type === 'off') {
